...
static ssize_t file_read_callback(nghttp2_session *session, int32_t stream_id,
  ...
  fprintf(stderr, "%s called fd %d length %ld\n", __func__, fd, length);
...
static int error_reply(nghttp2_session *session,
  ...
  ssize_t writelen;
  int pipefd[2];
  nghttp2_nv hdrs[] = {MAKE_NV(":status", "404", strlen("404"))};
...
static int on_request_recv(nghttp2_session *session,
                           ...
                           http2_stream_data *stream_data) {
  int fd;
  nghttp2_nv hdrs[] = {MAKE_NV(":status", "200", strlen("200"))};
  char *rel_path;
...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    fprintf(stderr, "Usage: server PORT KEY_FILE CERT_FILE\n");
    exit(EXIT_FAILURE);
  }
