...
static http2_session_data *create_http2_session_data(app_context_t *app_ctx,
		int fd,
...
static int on_header_callback(nghttp2_session *session,
	...
	switch (frame->hd.type) {
		case NGHTTP2_HEADERS:
			...
			/* assign stream. if already used of already path setted (same), break */
			stream_data =
			...
			/* check header name and save value to stream */
			/* if hdr name path:, remove query and save to stream */
			if (namelen == sizeof(PATH) - 1 && memcmp(PATH, name, namelen) == 0) {
				...
				sprintf(stream_data->method, "%s", header_value);
			} else if (!strcmp(header_name, HDR_CONTENT_TYPE)) {
				sprintf(stream_data->content_type, "%s", header_value);
			} else {
				fprintf(stderr, "dbg} skipped!\n");
			}
...
/* Minimum check for directory traversal. Returns nonzero if it is
   safe. */
#if 0 // useless because we check & use define path only
	...
	switch (frame->hd.type) {
		...
		case NGHTTP2_HEADERS:
			...
			if (frame->hd.flags & NGHTTP2_FLAG_END_STREAM) {
				...
				if (stream_data) {
					print_stream_data(stream_data);
				} else {
					return 0;
				}
				/* check stream_data->path
				 * if not support reply 404 */
				if (!strcmp(stream_data->request_path, "/oauth2/token")) {
					on_request_recv_nrf(session, session_data, stream_data);
				} else {
					/* not supported path */
					send_response(session, stream_data->stream_id, hdr_err_notfound, 1, NULL);
				}
...
static int on_data_chunk_recv_callback(nghttp2_session *session, uint8_t flags,
	...
	if (stream_data) {
		if ((len + stream_data->body_len) > MAX_BODY_LEN) {
			...
			stream_data->body_len = stream_data->body_len + len;
		}
	} else {
		fprintf(stderr, "%s) h2 get stream fail", __func__);
	}
...
static void initialize_app_context(app_context_t *app_ctx, SSL_CTX *ssl_ctx,
